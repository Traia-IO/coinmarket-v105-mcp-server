#!/usr/bin/env python3
"""
Coinmarket v105 MCP Server - FastMCP with D402 Transport Wrapper
...
- COINMARKET_V105_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('coinmarket-v105_mcp')
...
API_KEY = os.getenv("COINMARKET_V105_API_KEY")
if not API_KEY:
    logger.warning(f"⚠️  COINMARKET_V105_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"Coinmarket v105 MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("Coinmarket v105 MCP Server", host="0.0.0.0")
...
# - COINMARKET_V105_API_KEY: Server's internal API key for payment mode
#
...
async def coinmarketcap_id_map_7zgv(
    context: Context,
    listing_status: str = "active",
    slug: Optional[str] = None,
    start: int = 1,
    ...
    Example Usage:
        await coinmarketcap_id_map_7zgv()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_7zgv: {e}")
...
async def coinmarketcap_id_map_wcq3(
    context: Context,
    start: int = 1,
    limit: Optional[int] = None,
    sort: str = "id",
    ...
    Example Usage:
        await coinmarketcap_id_map_wcq3()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_wcq3: {e}")
...
async def quotes_historical_omjq(
    context: Context,
    time_start: Optional[str] = None,
    time_end: Optional[str] = None,
    count: float = 10,
    ...
    try:
        ...
        logger.error(f"Error in quotes_historical_omjq: {e}")
...
async def quotes_latest_s0d5(
    ...
    try:
        ...
        logger.error(f"Error in quotes_latest_s0d5: {e}")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="coinmarket-v105-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "coinmarket-v105-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Coinmarket v105 MCP Server")
    logger.info("="*80)
