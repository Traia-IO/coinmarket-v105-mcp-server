...
This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Coinmarket v105 API. It enables AI agents and LLMs to interact with Coinmarket v105 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Coinmarket v105 endpoints
...
- **Coinmarket v105 Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **API Documentation**: []()
...
*Note: Replace `example_tool` with actual Coinmarket v105 API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/coinmarket-v105-mcp-server.git
   cd coinmarket-v105-mcp-server
   ```

2. Set your API key:
   ```bash
   export COINMARKET_V105_API_KEY="your-api-key-here"
   ```
...
2. Run the server:
   ```bash
COINMARKET_V105_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Coinmarket v105 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_COINMARKET_V105_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Coinmarket v105 API key
2. Server uses client's API key to call Coinmarket v105 API
3. No payment required
4. Client pays Coinmarket v105 directly
...
6. Server uses its INTERNAL Coinmarket v105 API key to call the API
...
```bash
# Required
COINMARKET_V105_API_KEY=your_internal_coinmarket-v105_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Coinmarket v105 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Coinmarket v105 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/coinmarket-v105-mcp-server",
  "mcp_server": {
    "name": "coinmarket-v105-mcp",
    "description": "Another mcp from https://test-scan.d402.net",
...
- `COINMARKET_V105_API_KEY`: Your Coinmarket v105 API key (required)
## Troubleshooting
